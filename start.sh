...
docker run -itd --volumes-from data --name server tomatosliu/ubuntu_sc:0.1 java -cp rmi.jar /data/PingPongServer $1
docker run -itd --volumes-from data --link server --name client tomatosliu/ubuntu_sc:0.1 java -cp rmi.jar /data/PingPongClient $1

# Some sort of test, print the log and see the result and verification
echo "=============================build completed============================="
echo ""
echo 'Running the communication between client and server... (about 30s)'
sleep 31s
